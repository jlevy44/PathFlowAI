...
@click.option(
    ...
    "--img2npy",
    is_flag=True,
    help="Image to numpy for faster read.",
    show_default=True,
)
@click.option(
    "-b",
...
@click.option(
    ...
    default=[],
    multiple=True,
    help="Annotations in image in order.",
    type=click.Path(exists=False),
    show_default=True,
)
@click.option(
...
@click.option(
    "-pa", "--patches", is_flag=True, help="Add patches to SQL.", show_default=True
)
@click.option(
    "-t",
    "--threshold",
    default=0.05,
    help="Threshold to remove non-purple slides.",
    show_default=True,
)
...
@click.option(
    ...
    "--intensity_threshold",
    default=100.0,
    help="Intensity threshold to rate a pixel as non-white.",
    show_default=True,
)
@click.option(
    "-g",
    ...
    is_flag=True,
    help="Generate patches for one segmentation mask class for targeted finetuning.",
    show_default=True,
)
@click.option(
    "-tc",
    ...
    default=0,
    help="Segmentation Class to finetune on, output patches to another db.",
    show_default=True,
)
@click.option(
    "-tt",
...
@click.option(
    ...
    "--adjust_mask",
    is_flag=True,
    help="Remove additional background regions from annotation mask.",
    show_default=True,
)
@click.option(
    "-nn",
    "--n_neighbors",
    default=5,
    help="If adjusting mask, number of neighbors connectivity to remove.",
    show_default=True,
)
@click.option(
    "-bp",
    "--basic_preprocess",
    is_flag=True,
    help="Basic preprocessing pipeline, annotation areas are not saved. Used for benchmarking tool against comparable pipelines",
    show_default=True,
)
@click.option(
    "-ei",
    "--entire_image",
    is_flag=True,
    help="Store entire image in central db rather than patches.",
    show_default=True,
)
@click.option(
    "-nz",
    "--no_zarr",
    is_flag=True,
    help="Don't save zarr format file.",
    show_default=True,
)
def preprocess_pipeline(
    ...
    """Preprocessing pipeline that stores into ZARR format, performs optional mask adjustment, and stores patch-level information into SQL DB."""
    ...
    if npy_mask is None and xml_file is None:
    ...
    if adjust_mask:
        ...
        if not os.path.exists(adj_npy):
            adjust_mask(
                npy_mask, out_zarr if not no_zarr else svs_file, adj_npy, n_neighbors
            )
...
@click.option(
    ...
    "--mask_dir",
    default="./inputs/",
    help="Input directory for masks.",
    type=click.Path(exists=False),
    show_default=True,
)
@click.option(
    ...
    "--output_dir",
    default="./outputs/",
    help="Output directory for new masks.",
    type=click.Path(exists=False),
    show_default=True,
)
@click.option(
    ...
    default=[],
    multiple=True,
    help="Annotations to switch from.",
    show_default=True,
)
@click.option(
    "-to",
    ...
    dask.config.set(
        {
            ...
            # 'distributed.worker.num-workers':20
        }
    )
