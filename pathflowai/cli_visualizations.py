...
@click.option(
    ...
    help="Datbase containing all patches",
    type=click.Path(exists=False),
    show_default=True,
)
...
@click.option(
    ...
    help="Output extracted image.",
    type=click.Path(exists=False),
    show_default=True,
)
@click.option(
    "-s", "--segmentation", is_flag=True, help="Plot segmentations.", show_default=True
)
@click.option(
    "-sc",
    "--n_segmentation_classes",
    default=4,
    help="Number segmentation classes",
    show_default=True,
)
@click.option(
    "-c",
    "--custom_segmentation",
    default="",
    help="Add custom segmentation map from prediction, in npy",
    show_default=True,
)
def extract_patch(
    input_dir,
...
def plot_image(image_file, compression_factor, outputfname):
    ...
    plot_image_(
        image_file, compression_factor=compression_factor, test_image_name=outputfname
    )
...
@click.option(
    ...
    "--annotations",
    is_flag=True,
    help="Plot annotations instead of predictions.",
    show_default=True,
)
@click.option(
    "-cf",
...
@click.option(
    ...
    "--alpha",
    default=0.8,
    help="How much to give annotations/predictions versus original image.",
    show_default=True,
)
@click.option(
    "-s", "--segmentation", is_flag=True, help="Plot segmentations.", show_default=True
...
@click.option(
    ...
    "--custom_segmentation",
    default="",
    help="Add custom segmentation map from prediction, npy format.",
    show_default=True,
)
@click.option(
    "-ac",
...
@click.option(
    ...
    "--scaling_factor",
    default=1.0,
    help="Multiply all prediction scores by this amount.",
    show_default=True,
)
@click.option(
    "-tif", "--tif_file", is_flag=True, help="Write to tiff file.", show_default=True
)
...
def overlay_new_annotations(
    ...
    im = (
        ...
        else np.load(img_file, mmap_mode="r+")
    )
    print(im.shape)
    if compression_factor > 1 and original_compression_factor == 1.0:
        # im.resize((int(im.shape[0]/compression_factor),int(im.shape[1]/compression_factor)))
        im = cv2.resize(
            im,
            dsize=(
                int(im.shape[1] / compression_factor),
                int(im.shape[0] / compression_factor),
            ),
    ...
    with open(annotation_txt) as f:
        polygons = [
            np.array(
                [
                    list(map(float, filter(None, coords.strip(" ").split(","))))
                    for coords in re.sub("\]|\[|\ ", "", line).rstrip().split("Point:")
                    if coords
                ]
            )
            / compression_factor
...
@click.option(
    "-i",
    "--embeddings_file",
    default="predictions/embeddings.pkl",
    help="Embeddings.",
    type=click.Path(exists=False),
    show_default=True,
)
@click.option(
    "-o",
    "--plotly_output_file",
    default="predictions/embeddings.html",
    help="Plotly output file.",
    type=click.Path(exists=False),
    show_default=True,
)
@click.option(
    ...
    default=[],
    multiple=True,
    help="Multiple annotations to color image.",
    show_default=True,
)
@click.option(
    "-rb",
...
@click.option(
    ...
    "--max_background_area",
    default=0.05,
    help="Max background area before exclusion.",
    show_default=True,
)
@click.option(
    "-b",
...
def plot_embeddings(
    ...
    if remove_background_annotation:
        removal_bool = (
            patch_info[remove_background_annotation] <= (1.0 - max_background_area)
        ).values
    ...
    if annotations:
        ...
        if len(annotations) > 1:
            embeddings.loc[:, "ID"] = np.vectorize(
                lambda i: annotations[np.argmax(patch_info.iloc[i][annotations].values)]
    ...
    t_data = pd.DataFrame(
        umap.fit_transform(embeddings.iloc[:, :-1].values),
        columns=["x", "y", "z"],
...
@click.option(
    "-o",
    "--outputfilename",
    default="predictions/shap_plots.png",
    help="SHAPley visualization.",
    type=click.Path(exists=False),
    show_default=True,
)
@click.option(
    ...
    "--method",
    default="deep",
    help="Method of explaining.",
    type=click.Choice(["deep", "gradient"]),
    show_default=True,
)
@click.option(
    "-l",
    "--local_smoothing",
    default=0.0,
    help="Local smoothing of SHAP scores.",
    show_default=True,
)
@click.option(
    "-ns",
    "--n_samples",
    default=32,
    help="Number shapley samples for shapley regression (gradient explainer).",
    show_default=True,
)
@click.option(
    "-p",
    "--pred_out",
    default="none",
    help="If not none, output prediction as shap label.",
    type=click.Choice(["none", "sigmoid", "softmax"]),
    show_default=True,
)
def shapley_plot(
    model_pkl,
    ...
    model_dict["dataset_opts"]["transformers"] = get_data_transforms(
        **model_dict["transform_opts"]
    )
    plot_shap(
        model_dict["model"],
        model_dict["dataset_opts"],
        model_dict["transform_opts"],
        batch_size,
        outputfilename,
        method=method,
...
@click.option(
    ...
    "--outputfilename",
    default="predictions/shap_plots.png",
    help="Embedding visualization.",
    type=click.Path(exists=False),
    show_default=True,
)
@click.option(
    "-mpl", "--mpl_scatter", is_flag=True, help="Plot segmentations.", show_default=True
)
@click.option(
    "-rb",
    "--remove_background_annotation",
...
@click.option(
    ...
    "--sort_col",
    default="",
    help="Sort samples on this column.",
    type=click.Path(exists=False),
    show_default=True,
)
@click.option(
...
def plot_image_umap_embeddings(
    input_dir,
    embeddings_file,
    basename,
    outputfilename,
