        ...
        if t_data_df[color_col].dtype == np.float64:
            plots.append(
                go.Scatter3d(
                    ...
                    mode="markers",
                    marker=dict(
                        color=t_data_df[color_col],
                        size=size,
                        opacity=opacity,
                        colorscale="Viridis",
                        colorbar=dict(title="Colorbar"),
                    ),
                    text=t_data_df[color_col]
                    if name_col not in list(t_data_df)
                    else t_data_df[name_col],
                )
            )
        else:
            ...
            c = np.array(
                [
                    "rgb({})".format(
                        ",".join(
                            ((np.array(c_i) * 255).astype(int).astype(str).tolist())
                        )
                    )
                    for c_i in c
                ]
            ...
            for name, col in color_dict.items():
                plots.append(
                    go.Scatter3d(
                        x=t_data_df[x][t_data_df[color_col] == name],
                        y=t_data_df[y][t_data_df[color_col] == name],
                        z=t_data_df[z][t_data_df[color_col] == name],
                        name=str(name),
                        mode="markers",
                        marker=dict(color=col, size=size, opacity=opacity),
                        text=t_data_df.index[t_data_df[color_col] == name]
                        if "name" not in list(t_data_df)
                        else t_data_df[name_col][t_data_df[color_col] == name],
                    )
                )
        if G is not None:
            ...
            for edge in G.edges():
                if (
                    edge[0] in t_data_df.index.values
                    and edge[1] in t_data_df.index.values
                ):
...
def blend(arr1, arr2, alpha=0.5):
    """Blend 2 arrays together, mixing with alpha.
        ...
        """
    return alpha * arr1 + (1.0 - alpha) * arr2
...
def plot_image_(image_file, compression_factor=2.0, test_image_name="test.png"):
    ...
    if image_file.endswith(".zarr"):
        ...
        arr = (
            ...
            else npy2da(image_file)
        )
    arr2 = to_pil(
        cv2.resize(
            arr.compute(),
            dsize=tuple(
                (np.array(arr.shape[:2]) / compression_factor).astype(int).tolist()
            ),
...
class PredictionPlotter:
    ...
    def __init__(
        ...
        if not no_db:
            ...
            self.annotations = {
                str(a): i
                for i, a in enumerate(patch_info["annotation"].unique().tolist())
            }
            ...
            self.patch_info = self.patch_info[
                np.isin(self.patch_info["ID"], np.array(list(dask_arr_dict.keys())))
            ]
    ...
    def generate_image(self, ID):
        ...
        for i in range(patch_info.shape[0]):
            ...
            x_new, y_new = (
                int(x / self.compression_factor),
                int(y / self.compression_factor),
            )
            image = np.zeros((patch_size, patch_size, 3))
            if self.segmentation:
                image = seg2rgb(
                    self.segmentation_maps[ID][
                ...
                image = (
                    prob2rbg(pred * self.scaling_factor, self.pred_palette, image)
                    if not self.plot_annotation
                    else annotation2rgb(
                        self.annotations[str(pred)], self.palette, image
                    )
                )  # annotation
    ...
    def return_patch(self, ID, x, y, patch_size):
        """Return one single patch instead of entire image.
                ...
                """
        img = (
            self.dask_arr_dict[ID][x : x + patch_size, y : y + patch_size].compute()
...
def plot_shap(
    ...
    dataloader_val = DataLoader(
        ...
        sampler=ImbalancedDatasetSampler(dataset) if num_targets == 1 else None,
    )
    # dataloader_test = DataLoader(
    #     dataset, batch_size=batch_size, num_workers=10, shuffle=False)
    ...
    if method == "deep":
        ...
        e = shap.GradientExplainer(
            model, background, batch_size=batch_size, local_smoothing=local_smoothing
        )
    ...
    if pred_out != "none":
        ...
        labels = np.array(
            [
                [
                    (
                        dataloader_val.dataset.targets[i[j]]
                        if num_targets > 1
                        else str(i)
                    )
...
def plot_umap_images(
    ...
    cval=1.0,
    image_res=300.0,
    outputfname="output_embedding.png",
    mpl_scatter=True,
    remove_background_annotation="",
    max_background_area=0.01,
    zoom=0.05,
    n_neighbors=10,
    sort_col="",
    sort_mode="asc",
    ...
    if mpl_scatter:
        ...
        images = [min_resize(image, image_res) for image in images]
