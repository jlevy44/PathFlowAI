...
import pandas as pd
import numpy as np
from pathflowai.utils import npy2da, modify_patch_info, load_sql_df
...
def get_data_transforms(
    ...
    transform_dict = dict(
        ...
        albumentations=dict(
            huesaturation=lambda kargs: alb.augmentations.transforms.HueSaturationValue(
                **kargs
            ),
            ...
            elastic=lambda kargs: alb.augmentations.transforms.ElasticTransform(
                **kargs
            ),
        ),
    )
    ...
    main_transforms = (
        default_transforms[transform_platform]
        if not user_transforms
        else user_transforms
    )
    print(main_transforms)
    train_transforms = [
        transform_dict[transform_platform][k](v) for k, v in main_transforms.items()
    ]
    torch_init = [
    ...
    tensor_norm = [
        ...
        transforms.Normalize(
            mean if mean else [0.7, 0.6, 0.7],
            std if std is not None else [0.15, 0.15, 0.15],
        ),
...
def get_normalizer(normalization_file, dataset_opts):
    ...
    if "normalization_file" in norm_dict:
        ...
        transformers = get_data_transforms(
            patch_size=224, mean=[], std=[], resize=True, transform_platform="torch"
        )
        ...
        # nc.SafeDataset(DynamicImageDataset(**dataset_opts))
        ...
        all_mean = torch.tensor([0.0, 0.0, 0.0], dtype=torch.float)  # []

        all_std = torch.tensor([0.0, 0.0, 0.0], dtype=torch.float)
...
def segmentation_transform(img, mask, transformer, normalizer, alb_reduction):
    ...
    # .view(res_mask_shape[0],res_mask_shape[1],res_mask_shape[2])
    return (
        normalizer(
            torch.tensor(
                np.transpose(res["image"] / alb_reduction, axes=(2, 0, 1)),
                dtype=torch.float,
            )
        ).float(),
        torch.tensor(res["mask"]).long(),
    )
            ...
            for k in self.dilation_jitter:
                amount_jitter = int(
                    round(
                        max(
                            np.random.normal(
                                self.dilation_jitter[k]["mean"],
                                self.dilation_jitter[k]["std"],
                            ),
                            1,
                        )
                    )
                ...
                mask[
                    binary_dilation(
                        mask == k, structure=self.struct, iterations=amount_jitter
                    )
                ] = k
...
class DynamicImageDataset(Dataset):
    ...
    def __init__(
        ...
        self.alb_reduction = 255.0 if reduce_alb else 1.0
        ...
        if original_set == "pass":
            ...
            if self.segmentation:
                self.transform_fn = lambda x, y: segmentation_transform(
                    x, y, self.transformer, self.alb_normalizer, self.alb_reduction
                )
            else:
                if "p" in dir(self.transformer):
                    self.transform_fn = lambda x, y: (
                        self.alb_normalizer(
                            torch.tensor(
                                np.transpose(
                                    self.transformer(True, image=x)["image"]
                                    / self.alb_reduction,
                                    axes=(2, 0, 1),
                                ),
                                dtype=torch.float,
        ...
        if self.segmentation:
            self.targets = "target"
            self.image_set[self.targets] = 1.0
        if not self.segmentation and fix_names:
            self.image_set.loc[:, "ID"] = self.image_set["ID"].map(fix_names)
        self.slide_info = pd.DataFrame(
            self.image_set.set_index("ID").loc[:, self.targets]
        )
        ...
        if oversampling_factor > 1:
            self.patch_info = pd.concat(
                [self.patch_info] * int(oversampling_factor), axis=0
            ).reset_index(drop=True)
        elif oversampling_factor < 1:
            self.patch_info = self.patch_info.sample(
                frac=oversampling_factor
            ).reset_index(drop=True)
        ...
        print(self.targets)
        self.dilation_jitter = DilationJitter(
            dilation_jitter, self.segmentation, (original_set == "train")
        )
        if not self.targets:
            self.targets = [pos_annotation_class] + list(other_annotations)
    ...
    def concat(self, other_dataset):
        ...
        """
        self.patch_info = pd.concat(
            [self.patch_info, other_dataset.patch_info], axis=0
        ).reset_index(drop=True)
        self.length = self.patch_info.shape[0]
        if self.segmentation:
    ...
    def get_class_weights(self, i=0):  # [0,1]
        ...
        if self.segmentation:
            label_counts = (
                self.patch_info[
                    list(map(str, list(range(self.n_segmentation_classes))))
                ]
                .sum(axis=0)
                .values
            )
            ...
            weights = 1.0 / (self.patch_info[self.targets].sum(axis=0).values)
    ...
    def binarize_annotations(self, binarizer=None, num_targets=1, binary_threshold=0.0):
        ...
        if not self.mt_bce and num_targets > 1:
            ...
            annotation_labels = pd.DataFrame(
                self.binarizer.transform(annotations),
                index=self.patch_info.index,
            ...
            if binary_threshold > 0.0:
                self.patch_info.loc[:, self.targets] = (
                    self.patch_info[self.targets] >= binary_threshold
    ...
    def update_dataset(self, input_dir, new_db, prediction_basename=[]):
        ...
        IDs = self.patch_info["ID"].unique()
        self.slides = {
            slide: da.from_zarr(join(self.input_dir, "{}.zarr".format(slide)))
            for slide in IDs
        }
        if self.segmentation:
            if prediction_basename:
                self.segmentation_maps = {
                    slide: npy2da(join(self.input_dir, "{}_mask.npy".format(slide)))
                    for slide in IDs
                    if slide in prediction_basename
                }
            else:
                self.segmentation_maps = {
                    slide: npy2da(join(self.input_dir, "{}_mask.npy".format(slide)))
    ...
    def __getitem__(self, i):
        ...
        # .unsqueeze(0) # transpose .transpose([1,0,2])
        image, y = self.transform_fn(arr.compute().astype(np.uint8), y)
        if (
            not self.segmentation
            and not self.mt_bce
            and self.classify_annotations
            and use_long
        ):
            y = y.long()
