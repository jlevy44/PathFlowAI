...
def train_model_(training_opts):
	...
	transformers = get_data_transforms(patch_size = 224, mean=norm_dict['mean'], std=norm_dict['std'], resize=True)
	...
	model = generate_model(pretrain=training_opts['pretrain'],num_layers=training_opts['num_layers'],num_classes=training_opts['num_targets'], add_sigmoid=True, n_hidden=training_opts['n_hidden'])
