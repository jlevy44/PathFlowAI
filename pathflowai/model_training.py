...
from pathflowai.models import ModelTrainer, generate_model
...
def train_model_(training_opts):
    ...
    dataset_df = (
        pd.read_csv(training_opts["dataset_df"])
        if os.path.exists(training_opts["dataset_df"])
        else create_train_val_test(
            training_opts["train_val_test_splits"],
            training_opts["patch_info_file"],
            training_opts["patch_size"],
        )
    )
    ...
    dataset_opts = dict(
        ...
        set="pass",
        patch_info_file=training_opts["patch_info_file"],
        input_dir=training_opts["input_dir"],
        target_names=training_opts["target_names"],
        pos_annotation_class=training_opts["pos_annotation_class"],
        segmentation=training_opts["segmentation"],
        patch_size=training_opts["patch_size"],
        fix_names=training_opts["fix_names"],
        other_annotations=training_opts["other_annotations"],
        target_segmentation_class=training_opts["target_segmentation_class"][0]
        if set == "train"
        else -1,
        target_threshold=training_opts["target_threshold"][0],
        oversampling_factor=training_opts["oversampling_factor"][0]
        if set == "train"
        else 1,
        n_segmentation_classes=training_opts["num_targets"],
    ...
    transform_opts = dict(
        ...
        std=norm_dict["std"],
        resize=True,
        transform_platform=training_opts["transform_platform"]
        if not training_opts["segmentation"]
        else "albumentations",
        user_transforms=training_opts["user_transforms"],
    )

    transformers = get_data_transforms(**transform_opts)

    datasets = {
        set: DynamicImageDataset(
            dataset_df,
            set,
            training_opts["patch_info_file"],
            transformers,
            training_opts["input_dir"],
            training_opts["target_names"],
            training_opts["pos_annotation_class"],
            segmentation=training_opts["segmentation"],
            patch_size=training_opts["patch_size"],
            fix_names=training_opts["fix_names"],
            other_annotations=training_opts["other_annotations"],
    ...
    if len(training_opts["target_segmentation_class"]) > 1:
        ...
        for i in range(1, len(training_opts["target_segmentation_class"])):
            ...
            datasets["train"].concat(
                DynamicImageDataset(
                    ...
                    fix_names=training_opts["fix_names"],
                    other_annotations=training_opts["other_annotations"],
                    target_segmentation_class=training_opts[
                        "target_segmentation_class"
                    ][i],
                    target_threshold=training_opts["target_threshold"][i],
                    oversampling_factor=training_opts["oversampling_factor"][i],
                    n_segmentation_classes=training_opts["num_targets"],
                    gdl=training_opts["loss_fn"] == "gdl",
                    mt_bce=training_opts["mt_bce"],
                    classify_annotations=training_opts["classify_annotations"],
        ...
        # datasets['train'] = reduce(lambda x, y: x.concat(y), [DynamicImageDataset(dataset_df, 'train', training_opts['patch_info_file'], transformers, training_opts['input_dir'], training_opts['target_names'], training_opts['pos_annotation_class'], segmentation=training_opts['segmentation'], patch_size=training_opts['patch_size'],
        #                                                                           fix_names=training_opts['fix_names'], other_annotations=training_opts['other_annotations'], target_segmentation_class=training_opts['target_segmentation_class'][i], target_threshold=training_opts['target_threshold'][i], oversampling_factor=training_opts['oversampling_factor'][i]) for i in range(len(training_opts['target_segmentation_class']))])
        print(datasets["train"])

    if training_opts["supplement"]:
        old_train_set = copy.deepcopy(datasets["train"])
        datasets["train"] = DynamicImageDataset(
            dataset_df,
            "train",
            training_opts["patch_info_file"],
            transformers,
            ...
            target_threshold=training_opts["target_threshold"],
    ...
    if not training_opts["predict"]:
        ...
        if training_opts["segmentation"]:
            for ID, dataset in (
                ...
                else datasets["test"].select_IDs(training_opts["prediction_basename"])
            ):
            ...
            if extract_embedding:
                ...
                patch_info.to_sql(
                    str(training_opts["patch_size"]), con=conn, if_exists=("replace")
                )
                conn.close()
...
@train.command()
@click.option(
    "-s", "--segmentation", is_flag=True, help="Segmentation task.", show_default=True
)
@click.option(
    "-p", "--prediction", is_flag=True, help="Predict on model.", show_default=True
)
@click.option(
    "-pa",
    "--pos_annotation_class",
    default="",
    help="Annotation Class from which to apply positive labels.",
...
@click.option(
    ...
    "--save_location",
    default="",
    help="Model Save Location, append with pickle .pkl.",
    type=click.Path(exists=False),
    show_default=True,
)
@click.option(
    "-pt",
    "--pretrained_save_location",
    default="",
    help="Model Save Location, append with pickle .pkl, pretrained by previous analysis to be finetuned.",
    type=click.Path(exists=False),
    show_default=True,
)
@click.option(
    ...
    "--input_dir",
    default="",
    help="Input directory containing slides and everything.",
    type=click.Path(exists=False),
    show_default=True,
)
@click.option("-ps", "--patch_size", default=224, help="Patch size.", show_default=True)
@click.option(
    "-pr", "--patch_resize", default=224, help="Patch resized.", show_default=True
)
@click.option(
    "-tg",
    "--target_names",
...
@click.option(
    ...
    "--dataset_df",
    default="",
    help="CSV file with train/val/test and target info.",
    type=click.Path(exists=False),
    show_default=True,
)
@click.option(
    ...
    "--fix_names",
    is_flag=True,
    help="Whether to fix names in dataset_df.",
    show_default=True,
)
@click.option(
    "-a",
    ...
    type=click.Choice(
        ...
        + ["efficientnet-b{}".format(i) for i in range(8)]
    ),
    show_default=True,
)
@click.option(
    ...
    is_flag=True,
    help="Attempt to correct for imbalanced data.",
    show_default=True,
)
@click.option(
    "-imb2",
    "--imbalanced_correction2",
    is_flag=True,
    help="Attempt to correct for imbalanced data.",
    show_default=True,
)
@click.option(
    ...
    is_flag=True,
    help="Classify annotations.",
    show_default=True,
)
@click.option(
    "-nt", "--num_targets", default=1, help="Number of targets.", show_default=True
)
@click.option(
    "-ss",
    "--subsample_p",
    default=1.0,
    help="Subsample training set.",
    show_default=True,
)
@click.option(
    "-ssv",
    "--subsample_p_val",
    default=-1.0,
    help="Subsample val set. If not set, defaults to that of training set",
    show_default=True,
)
@click.option(
    "-t",
    "--num_training_images_epoch",
    default=-1,
    help="Number of training images per epoch. -1 means use all training images each epoch.",
    show_default=True,
)
@click.option(
    "-lr", "--learning_rate", default=1e-2, help="Learning rate.", show_default=True
)
@click.option(
    "-tp",
    "--transform_platform",
    default="torch",
    help="Transform platform for nonsegmentation tasks.",
    type=click.Choice(["torch", "albumentations"]),
)
@click.option(
    "-ne", "--n_epoch", default=10, help="Number of epochs.", show_default=True
)
@click.option(
    "-pi",
    "--patch_info_file",
    default="patch_info.db",
    help="Patch info file.",
    type=click.Path(exists=False),
    show_default=True,
)
@click.option(
    ...
    default=[-1],
    multiple=True,
    help="Segmentation Class to finetune on.",
    show_default=True,
)
@click.option(
    "-tt",
    ...
    default=[0.0],
    multiple=True,
    help="Threshold to include target for segmentation if saving one class.",
    show_default=True,
)
@click.option(
    "-ov",
    ...
    default=[1.0],
    multiple=True,
    help="How much to oversample training set.",
    show_default=True,
)
@click.option(
    "-sup",
    "--supplement",
    is_flag=True,
    help="Use the thresholding to supplement the original training set.",
    show_default=True,
)
...
@click.option(
    ...
    "--run_test",
    is_flag=True,
    help='Output predictions for a batch to "test_predictions.npy". Use for debugging.',
    show_default=True,
)
@click.option(
    "-mtb",
    "--mt_bce",
    is_flag=True,
    help="Run multi-target bce predictions on the annotations.",
    show_default=True,
)
@click.option(
    "-po",
    ...
    default="predictions",
    help="Where to output segmentation predictions.",
    type=click.Path(exists=False),
    show_default=True,
)
@click.option(
...
@click.option(
    ...
    "--binary_threshold",
    default=0.0,
    help="If running binary classification on annotations, dichotomize selected annotation as such.",
    show_default=True,
)
@click.option(
    "-prt", "--pretrain", is_flag=True, help="Pretrain on ImageNet.", show_default=True
)
@click.option(
    "-olf",
    "--overwrite_loss_fn",
    default="",
    help="Overwrite the default training loss functions with loss of choice.",
    type=click.Choice(["", "bce", "mse", "focal", "dice", "gdl", "ce"]),
    show_default=True,
)
@click.option(
    "-atl",
    "--adopt_training_loss",
    is_flag=True,
    help="Adopt training loss function for validation calculation.",
    show_default=True,
)
@click.option(
    "-tdb",
    "--external_test_db",
    default="",
    help="External database of samples to test on.",
    type=click.Path(exists=False),
    show_default=True,
)
@click.option(
    ...
    "--external_test_dir",
    default="",
    help="External directory of samples to test on.",
    type=click.Path(exists=False),
    show_default=True,
)
@click.option(
    ...
    default=[""],
    multiple=True,
    help="For segmentation tasks, if supplied, can predict on these basenames rather than the entire test set. Only works for segmentation tasks for now",
    show_default=True,
)
@click.option(
    "-cw",
    "--custom_weights",
    default="",
    help="Comma delimited custom weights",
    type=click.Path(exists=False),
    show_default=True,
)
@click.option(
...
def train_model(
    ...
    oversampling_factor = [
        (int(x) if float(x) >= 1 else float(x)) for x in oversampling_factor
    ]
    ...
    segmentation_training_opts.update(
        dict(
    ...
    if user_transforms_file and os.path.exists(user_transforms_file):
        ...
        with open(user_transforms_file) as f:
            ...
            if "dilationjitter" in list(training_opts["user_transforms"].keys()):
                training_opts["dilation_jitter"] = training_opts["user_transforms"].pop(
                    "dilationjitter"
                )
