...
def get_boxes(m, ID="test", x="x", y="y", patch_size="patchsize", num_classes=3):
    ...
    boxes = dask.compute(
        *[
            dask.delayed(get_box)(rev_label[i], objProps[i - 1])
            for i in list(rev_label.keys())
        ],
        scheduler="threading"
    )  # [get_box(rev_label[i],objProps[i-1]) for i in list(rev_label.keys())]#
    # print(boxes)
    boxes = pd.DataFrame(
        np.array(boxes).astype(int),
        columns=["class_label", "x_top_left", "y_top_left", "width"],
    )
...
if __name__ == "__main__":
    ...
    patch_info = patch_info[
        ~np.isin(
            np.vectorize(
                lambda i: "/".join(
                    patch_info.iloc[i][["ID", "x", "y", "patch_size"]]
                    .astype(str)
                    .tolist()
                )
            )(np.arange(patch_info.shape[0])),
            set(bbox_df.image.cat.categories),
        )
    ]
    ...
    def get_boxes_point_seg(m, ID, x, y, patch_size2, num_classes):
        bbox_dff = get_boxes(
            m, ID=ID, x=x, y=y, patch_size=patch_size2, num_classes=num_classes
        )
        for i in official_widths.keys():
            bbox_dff.loc[bbox_dff["class_label"] == i, "width"] = int(
                official_widths[i]
            )
        bbox_dff.loc[:, "x_top_left"] = (
            bbox_dff.loc[:, "x_top_left"] - bbox_dff["width"] / 2.0
        ).astype(int)
        bbox_dff.loc[:, "y_top_left"] = (
            bbox_dff.loc[:, "y_top_left"] - bbox_dff["width"] / 2.0
        ).astype(int)
        bbox_dff.loc[:, "x_top_left"] = np.clip(
            bbox_dff.loc[:, "x_top_left"], 0, m.shape[1]
        )
        bbox_dff.loc[:, "y_top_left"] = np.clip(
            bbox_dff.loc[:, "y_top_left"], 0, m.shape[0]
        )
        return bbox_dff
    ...
    def process_chunk(patch_info_sub):
        ...
        for i in range(patch_info_sub.shape[0]):
            ...
            bbox_dff = get_boxes_point_seg(
                m, ID, x, y, patch_size2, num_classes
            )  # dask.delayed(get_boxes_point_seg)(m,ID,x,y,patch_size2)
            # print(bbox_dff)
